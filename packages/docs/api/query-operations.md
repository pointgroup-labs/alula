...
### **`get_user_earn_obligation`**

Get user's _Earn_ obligation (deposit-only).

```rust
fn get_user_earn_obligation(user: Address) -> Result<Obligation, MCError>
```

***

### **`get_user_multiply_pair_obligation`**

Get user's obligation for a specific leveraged pair.

```rust
fn get_user_multiply_pair_obligation(
    user: Address,
