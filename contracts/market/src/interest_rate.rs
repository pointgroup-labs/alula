...
impl Pool {
    ...
    pub fn accrue_interest(&mut self, e: &Env) -> Result<(), MCError> {
        ...
        if take_rate_accrual_part > 0 {
            // TODO: Check what to do with precision loss &
            // with cases where no beneficiaries present, yet take_rate is non-zero

            // Regarding the first - we can have an endpoint that allows for an admin
            // to take all such fees to the market itself, right?

            // Yep, distribute must do so, I think - we simply forget about them?
            if let Some(take_rate_beneficiaries) = &self.config.fee_config.take_rate_beneficiaries {
