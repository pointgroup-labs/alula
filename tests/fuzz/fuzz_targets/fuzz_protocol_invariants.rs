...
fuzz_target!(|input: ProtocolInvariantInput| {
    ...
    let utilization_ratio_limit_bps = input
        .utilization_ratio_limit_bps
        .max(optimal_utilization_ratio_bps + 1);
...
fn test_pool_config_invariants(config: &PoolConfig) {
    ...
    assert!(
        config.base_rate_per_second >= 0,
        "Base rate per second must be non-negative: {}",
        config.base_rate_per_second
    );

    // Invariant: Supply limit should be non-negative
    assert!(
        config.supply_limit >= 0,
        "Supply limit must be non-negative: {}",
        config.supply_limit
    );

    // Invariant: All BPS values should be within valid ranges (0-10000)
    assert!(
        config.optimal_utilization_ratio_bps > 0 && config.optimal_utilization_ratio_bps <= 10000,
        "Optimal utilization ratio must be between 0% and 100%: {}",
        config.optimal_utilization_ratio_bps
    );

    assert!(
        config.reserve_ratio_bps >= 0 && config.reserve_ratio_bps <= 10000,
        "Reserve ratio must be between 0% and 100%: {}",
        config.reserve_ratio_bps
    );

    assert!(
        config.liquidation_close_factor_bps > 0 && config.liquidation_close_factor_bps <= 10000,
        "Liquidation close factor must be between 0% and 100%: {}",
        config.liquidation_close_factor_bps
    );

    assert!(
        config.liquidation_incentive_bps >= 0 && config.liquidation_incentive_bps <= 10000,
        "Liquidation incentive must be between 0% and 100%: {}",
        config.liquidation_incentive_bps
    );

    assert!(
        config.utilization_ratio_limit_bps > 0 && config.utilization_ratio_limit_bps <= 10000,
        "Utilization ratio limit must be between 0% and 100%: {}",
        config.utilization_ratio_limit_bps
    );

    assert!(
        config.open_ltv_bps >= 0 && config.open_ltv_bps < 10000,
        "Open LTV must be between 0% and less than 100%: {}",
        config.open_ltv_bps
    );

    assert!(
        config.close_ltv_bps >= 0 && config.close_ltv_bps <= 10000,
        "Close LTV must be between 0% and 100%: {}",
        config.close_ltv_bps
    );
}
...
fn test_mathematical_invariants(config: &PoolConfig) {
    ...
    assert!(
        config.slope1 >= 0,
        "Slope1 must be non-negative: {}",
        config.slope1
    );
    assert!(
        config.slope2 >= 0,
        "Slope2 must be non-negative: {}",
        config.slope2
    );

    // Invariant: slope1 must be less than slope2 for kinked model to work
    assert!(
        config.slope1 < config.slope2,
        "Slope1 {} must be less than slope2 {} for kinked model",
        config.slope1,
        config.slope2
    );
    ...
    assert!(
        config.open_ltv_bps <= config.close_ltv_bps,
    ...
    assert!(
        config.liquidation_incentive_bps <= 5000,
        "Liquidation incentive {} should not exceed 50%",
        config.liquidation_incentive_bps
    );
}
