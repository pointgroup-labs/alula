...
use farms::{
    FarmConfig, FarmConfigUpdate, FarmsClient, FarmsContract, LockingMode, RewardCurvePoint,
    RewardScheduleCurve, TimeUnit,
};
use soroban_sdk::{
    Address, IntoVal,
...
fn test_stake_and_unstake() {
    ...
    // Initialize user
    fixture.farms_client.initialize_user(user, &farm_id);

    // Stake
    fixture.farms_client.stake(user, &farm_id, &1000);

    let user_state = fixture.farms_client.get_user_state(user, &farm_id);
    ...
    // Unstake
    let net_amount = fixture.farms_client.unstake(user, &farm_id, &500);
...
fn test_stake_with_warmup() {
    ...
    fixture.farms_client.refresh_user_state(user, &farm_id);
...
fn test_reward_accrual_and_harvest() {
    ...
    let pending = fixture.farms_client.get_pending_rewards(user, &farm_id);
    assert!(!pending.is_empty());

    // Harvest
    let initial_balance = fixture.reward_token_client.balance(user);
    let harvested = fixture.farms_client.harvest(user, &farm_id, &0);
...
fn test_early_withdrawal_penalty() {
    ...
    let net_amount = fixture.farms_client.unstake(user, &farm_id, &1000);
...
fn test_delegated_farm_with_set_stake() {
    ...
    fixture.farms_client.set_stake_delegated(user, &farm_id, &1000);
...
fn test_delegated_farm_stake_increase_and_decrease() {
    ...
    fixture.farms_client.set_stake_delegated(user, &farm_id, &1500);
    ...
    fixture.farms_client.set_stake_delegated(user, &farm_id, &500);
...
fn test_delegated_farm_user_count_tracking() {
    ...
    fixture.farms_client.set_stake_delegated(user, &farm_id, &0);
...
fn test_delegated_farm_deposit_cap() {
    ...
    fixture.farms_client.set_stake_delegated(user, &farm_id, &400);
    ...
    let result = fixture.farms_client.try_set_stake_delegated(user, &farm_id, &600);
...
fn test_delegated_farm_frozen_rejects_stake() {
    ...
    let result = fixture.farms_client.try_set_stake_delegated(user, &farm_id, &1000);
    assert!(result.is_err());
}
...
fn test_unstake_with_cooldown() {
    ...
    fixture.farms_client.unstake(user, &farm_id, &500);
    ...
    let result = fixture.farms_client.try_withdraw_unstaked(user, &farm_id);
    assert!(result.is_err());
    ...
    let withdrawn = fixture.farms_client.withdraw_unstaked(user, &farm_id);
...
fn test_harvest_all() {
    ...
    let initial_balance = fixture.reward_token_client.balance(user);
    let total_harvested = fixture.farms_client.harvest_all(user, &farm_id);
    let final_balance = fixture.reward_token_client.balance(user);
...
fn test_reward_user_once_airdrop() {
    ...
    fixture.farms_client.reward_user_once(user, &farm_id, &0, &500);
...
fn test_reward_user_once_disabled_fails() {
    ...
    let result = fixture.farms_client.try_reward_user_once(user, &farm_id, &0, &500);
    assert!(result.is_err());
}
...
fn test_withdraw_slashed_amount() {
    ...
    fixture.farms_client.unstake(user, &farm_id, &1000);
...
fn test_stake_minimum_amount() {
    ...
    fixture.farms_client.stake(user, &farm_id, &1);
...
fn test_stake_zero_fails() {
    ...
    fixture.farms_client.stake(user, &farm_id, &0);
}
...
fn test_withdraw_unstaked_one_second_before_cooldown_end() {
    ...
    // Should fail
    fixture.farms_client.withdraw_unstaked(user, &farm_id);
}
...
fn test_user_count_multiple_stake_unstake_cycles() {
    ...
    // First stake
    fixture.farms_client.stake(user, &farm_id, &500);
    let farm = fixture.farms_client.get_farm(&farm_id);
    assert_eq!(farm.num_users, 1);
...
fn test_multiple_users_count() {
    ...
    fixture.farms_client.initialize_user(user1, &farm_id);
    fixture.farms_client.initialize_user(user2, &farm_id);

    fixture.farms_client.stake(user1, &farm_id, &1000);
    let farm = fixture.farms_client.get_farm(&farm_id);
    assert_eq!(farm.num_users, 1);
    assert_eq!(farm.total_staked, 1000);

    fixture.farms_client.stake(user2, &farm_id, &2000);
    let farm = fixture.farms_client.get_farm(&farm_id);
    assert_eq!(farm.num_users, 2);
    assert_eq!(farm.total_staked, 3000);

    // User1 fully unstakes
    fixture.farms_client.unstake(user1, &farm_id, &1000);
    let farm = fixture.farms_client.get_farm(&farm_id);
    assert_eq!(farm.num_users, 1);
...
fn test_double_user_initialization_fails() {
    ...
    fixture.farms_client.initialize_user(user, &farm_id);
    fixture.farms_client.initialize_user(user, &farm_id); // Should fail
}
...
fn test_stake_to_nonexistent_farm_fails() {
    ...
    fixture.farms_client.stake(user, &fake_farm_id, &1000);
}
...
fn test_unstake_more_than_staked_fails() {
    ...
    fixture.farms_client.initialize_user(user, &farm_id);
    fixture.farms_client.stake(user, &farm_id, &1000);
    fixture.farms_client.unstake(user, &farm_id, &1500); // Should fail
}
...
fn test_proportional_rewards_with_unequal_stakes() {
    ...
    fixture.farms_client.stake(user1, &farm_id, &1000);
    fixture.farms_client.stake(user2, &farm_id, &3000);

    fixture.pass_time(100);

    let pending1 = fixture.farms_client.get_pending_rewards(user1, &farm_id);
    let pending2 = fixture.farms_client.get_pending_rewards(user2, &farm_id);
...
fn test_with_expiry_lock_all_users_same_unlock() {
    ...
    fixture.pass_time(500);
    fixture.farms_client.stake(user2, &farm_id, &1000);

    // User2 unstakes immediately after staking (500s into lock, 500s remaining)
    let net_amount2 = fixture.farms_client.unstake(user2, &farm_id, &1000);
    ...
    let net_amount1 = fixture.farms_client.unstake(user1, &farm_id, &1000);
...
fn test_min_claim_duration_prevents_rapid_claims() {
    ...
    // First harvest
    e.mock_all_auths_allowing_non_root_auth();
    fixture.farms_client.harvest(user, &farm_id, &0);
...
fn test_min_claim_duration_allows_claim_after_duration() {
    ...
    // First harvest
    e.mock_all_auths_allowing_non_root_auth();
    let first_harvest = fixture.farms_client.harvest(user, &farm_id, &0);
