...
fn test_manager_deploy_markets() {
    ...
    let market_address_1 =
        manager_client.deploy(&salt_1, &market_admin, &name_1, &oracle, &0, &0, &0, &true);
    ...
    let market_address_2 =
        manager_client.deploy(&salt_2, &market_admin, &name_2, &oracle, &0, &0, &0, &true);
...
fn test_manager_cannot_redeploy_market() {
    ...
    let name_1 = String::from_str(&e, "market_1");
    manager_client.deploy(&salt, &market_admin, &name_1, &oracle, &0, &0, &0, &false);
    ...
    assert!(
        manager_client
            .try_deploy(&salt, &market_admin, &name_2, &oracle, &0, &0, &0, &false)
            .is_err()
    );
}
